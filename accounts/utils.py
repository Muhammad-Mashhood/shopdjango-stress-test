...
def send_verification_email(user):
    ...
    subject = force_str(_('Verify your ShopDjango account'))
    message = force_str(_(
...
def send_password_reset_email(user, reset_url):
    ...
    subject = force_str(_('Reset your ShopDjango password'))
    message = force_str(_('Click the link to reset your password: %s')) % reset_url
    send_mail(
        subject=subject,
        message=message,
        from_email=settings.EMAIL_HOST_USER,
...
def get_user_display_name(user):
    ...
    if user.first_name and user.last_name:
        return force_str('%s %s' % (user.first_name, user.last_name))
    return force_str(user.email)
